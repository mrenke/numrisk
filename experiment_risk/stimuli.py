...
def _create_stimulus_array(win, n_dots, circle_radius, dot_radius, pos,
                           image):
    xys = _sample_dot_positions(n_dots, circle_radius, dot_radius) # array

    xys += np.array(pos)[np.newaxis, :] # adds same 2 (x/y) values to each row in array
                                        # ("broadcasting")
