## Needs:
# - CM (group or individual)
# - geodesic distance matrices for spatial filtering
# - reference network parcellation (here: Cole-Anticevic)
...
def main(sub,bids_folder = '/mnt_AdaBD_largefiles/Data/SMILE_Data/DNumRisk/ds-dnumrisk',
    ...
    # Load in CM
    cm_fn = op.join(source_folder, f'sub-{sub}_ses-1_task-magjudge_confspec-36Pscrub3runFD104_CM-unfiltered.npy')
    cm_f = np.load(cm_fn)

    cm_filtered = spatial_filtering(cm_f, bids_folder=bids_folder)

    if ref_name == 'standard-ColeAnticevic': # take normal cole anticevic atlas as reference
        ref_fn = op.join(bids_folder,'derivatives','networks_infomap', 'caNets_fsaverage5_mapping.npy') 
    elif ref_name == 'dnumrisk-average': # take consensus mapping as reference
        ...
        ref_fn = '/mnt_AdaBD_largefiles/Data/SMILE_Data/DNumRisk/ds-dnumrisk/derivatives/networks_infomap_full_01/sub-average_consensusMapping_confspec-36Pscrub3BPfilter.npy'
    ...
    if save_ind_thresh_labels:
        fn_mappings = op.join(target_folder, f'sub-{sub}_threshs-{conn_thresholds_string}_ref_name-{ref_name}precFuncMaps-allThresholds.npy')
